...
const EventEmitter$1 = require('node:events');
...
var cpFile$1 = {exports: {}};

var pEvent$3 = {exports: {}};
...
const pEvent$2 = (emitter, event, options) => {
...
pEvent$3.exports = pEvent$2;
// TODO: Remove this for the next major release
pEvent$3.exports.default = pEvent$2;

pEvent$3.exports.multiple = multiple;

pEvent$3.exports.iterator = (emitter, event, options) => {
	if (typeof options === 'function') {
		options = {filter: options};
	}
...
pEvent$3.exports.TimeoutError = pTimeout.TimeoutError;

var inherits = require$$0.inherits;

var NestedError$1 = function (message, nested) {
    this.nested = nested;
...
inherits(NestedError$1, Error);
NestedError$1.prototype.name = 'NestedError';


var nestedErrorStacks = NestedError$1;

const NestedError = nestedErrorStacks;

let CpFileError$2 = class CpFileError extends NestedError {
...
const checkPath$1 = pth => {
	if (process.platform === 'win32') {
		const pathHasInvalidWinCharacters = /[<>:"|?*]/.test(pth.replace(path$4.parse(pth).root, ''));
...
const makeDir$1 = async (input, options) => {
	...
	if (useNativeRecursiveOption && options.fs.mkdir === fs$3.mkdir) {
		const pth = path$4.resolve(input);
...
const pEvent$1 = pEvent$3.exports;
const CpFileError$1 = cpFileError;

const stat = promisify$1(fs$2.stat);
const lstat = promisify$1(fs$2.lstat);
const utimes = promisify$1(fs$2.utimes);
const chmod = promisify$1(fs$2.chmod);

fs$5.closeSync = fs$2.closeSync.bind(fs$2);
fs$5.createWriteStream = fs$2.createWriteStream.bind(fs$2);

fs$5.createReadStream = async (path, options) => {
	const read = fs$2.createReadStream(path, options);

	try {
		await pEvent$1(read, ['readable', 'end']);
	} catch (error) {
		throw new CpFileError$1(`Cannot read from \`${path}\`: ${error.message}`, error);
	}

	return read;
};

fs$5.stat = path => stat(path).catch(error => {
	throw new CpFileError$1(`Cannot stat path \`${path}\`: ${error.message}`, error);
});

fs$5.lstat = path => lstat(path).catch(error => {
	throw new CpFileError$1(`lstat \`${path}\` failed: ${error.message}`, error);
});

fs$5.utimes = (path, atime, mtime) => utimes(path, atime, mtime).catch(error => {
	throw new CpFileError$1(`utimes \`${path}\` failed: ${error.message}`, error);
});

fs$5.chmod = (path, mode) => chmod(path, mode).catch(error => {
	throw new CpFileError$1(`chmod \`${path}\` failed: ${error.message}`, error);
});
...
fs$5.statSync = path => {
	try {
		...
		throw new CpFileError$1(`stat \`${path}\` failed: ${error.message}`, error);
	}
};

fs$5.utimesSync = (path, atime, mtime) => {
	try {
		return fs$2.utimesSync(path, atime, mtime);
	} catch (error) {
		throw new CpFileError$1(`utimes \`${path}\` failed: ${error.message}`, error);
	}
};

fs$5.makeDir = (path, options) => makeDir(path, {...options, fs: fs$2}).catch(error => {
	throw new CpFileError$1(`Cannot create directory \`${path}\`: ${error.message}`, error);
});

fs$5.makeDirSync = (path, options) => {
	try {
		makeDir.sync(path, {...options, fs: fs$2});
	} catch (error) {
		throw new CpFileError$1(`Cannot create directory \`${path}\`: ${error.message}`, error);
	}
};

fs$5.copyFileSync = (source, destination, flags) => {
	try {
		fs$2.copyFileSync(source, destination, flags);
	} catch (error) {
		throw new CpFileError$1(`Cannot copy from \`${source}\` to \`${destination}\`: ${error.message}`, error);
	}
};
...
let ProgressEmitter$1 = class ProgressEmitter extends EventEmitter {
...
var progressEmitter = ProgressEmitter$1;

const path$3 = require$$0$4;
const {constants: fsConstants} = require$$0$3;
const pEvent = pEvent$3.exports;
const CpFileError = cpFileError;
const fs$1 = fs$5;
...
const cpFileAsync = async (source, destination, options, progressEmitter) => {
	...
	const readStream = await fs$1.createReadStream(source);
	await fs$1.makeDir(path$3.dirname(destination), {mode: options.directoryMode});
	const writeStream = fs$1.createWriteStream(destination, {flags: options.overwrite ? 'w' : 'wx'});
	...
	if (shouldUpdateStats) {
		...
		return Promise.all([
			fs$1.utimes(destination, stats.atime, stats.mtime),
			fs$1.chmod(destination, stats.mode)
		]);
	}
};
...
const cpFile = (sourcePath, destinationPath, options) => {
	...
	const progressEmitter = new ProgressEmitter(path$3.resolve(sourcePath), path$3.resolve(destinationPath));
...
cpFile$1.exports.sync = (source, destination, options) => {
	...
	const stat = fs$1.statSync(source);
	checkSourceIsFile(stat, source);
	fs$1.makeDirSync(path$3.dirname(destination), {mode: options.directoryMode});

	const flags = options.overwrite ? null : fsConstants.COPYFILE_EXCL;
	try {
		fs$1.copyFileSync(source, destination, flags);
	} catch (error) {
	...
	fs$1.utimesSync(destination, stat.atime, stat.mtime);
};
...
const applyBaseToPattern = (pattern, base) => isNegativePattern(pattern)
	? '!' + path$5.posix.join(base, pattern.slice(1))
	: path$5.posix.join(base, pattern);

const parseIgnoreFile = (file, cwd) => {
	const base = slash(path$5.relative(cwd, path$5.dirname(file.filePath)));
...
const toRelativePath = (fileOrDirectory, cwd) => {
	cwd = slash(cwd);
	if (path$5.isAbsolute(fileOrDirectory)) {
		if (slash(fileOrDirectory).startsWith(cwd)) {
			return path$5.relative(cwd, fileOrDirectory);
		}
...
const isIgnoredByIgnoreFilesSync = (patterns, options) => {
	...
	const files = paths.map(filePath => ({
		filePath,
		content: fs$6.readFileSync(filePath, 'utf8'),
	}));
...
const checkCwdOption = options => {
	...
	let stat;
	try {
		stat = fs$6.statSync(options.cwd);
	} catch {
		return;
	}
...
const createFilterFunction = isIgnored => {
	...
	return fastGlobResult => {
		...
		const pathKey = path$5.normalize(path);
...
class CpyError extends nestedErrorStacks {
	constructor(message, nested) {
		super(message, nested);
		Object.assign(this, nested);
		this.name = 'CpyError';
	...
	*/
	constructor(pattern, destination, options) {
		...
		if (
			...
			&& fs$6.existsSync(pattern)
			&& fs$6.lstatSync(pattern).isDirectory()
		) {
	...
	get name() {
		return path$5.basename(this.originalPath);
	}
	...
	get normalizedPath() {
		...
		if (normalized) {
			return path$5.isAbsolute(normalized) ? normalized : path$5.join(this.options.cwd, normalized);
		}
	...
	getMatches() {
		...
		if (this.options.ignoreJunk) {
			matches = matches.filter(file => isNotJunk(path$5.basename(file)));
		}
...
const defaultConcurrency = (os$1.cpus().length || 1) * 2; // eslint-disable-line unicorn/explicit-length-check
...
class Entry {
	...
	*/
	constructor(source, relativePath, pattern) {
		...
		@type {string}
		*/
		this.path = source.split('/').join(path$5.sep);

		/**
		@type {string}
		*/
		this.relativePath = relativePath.split('/').join(path$5.sep);
	...
	get name() {
		return path$5.basename(this.path);
	}

	get nameWithoutExtension() {
		return path$5.basename(this.path, path$5.extname(this.path));
	}

	get extension() {
		return path$5.extname(this.path).slice(1);
	}
}
...
const preprocessDestinationPath = ({entry, destination, options}) => {
	if (entry.pattern.hasMagic()) {
		if (options.flat) {
			if (path$5.isAbsolute(destination)) {
				return path$5.join(destination, entry.name);
			}

			return path$5.join(options.cwd, destination, entry.name);
		}

		return path$5.join(
			destination,
			path$5.relative(entry.pattern.normalizedPath, entry.path),
		);
	}
	...
	if (entry.pattern.isDirectory && path$5.relative(options.cwd, entry.path).startsWith('..')) {
		return path$5.join(options.cwd, destination, path$5.basename(entry.pattern.originalPath), path$5.relative(entry.pattern.originalPath, entry.path));
	}
	...
	if (!entry.pattern.isDirectory && options.flat) {
		return path$5.join(options.cwd, destination, path$5.basename(entry.pattern.originalPath));
	}

	return path$5.join(options.cwd, destination, path$5.relative(options.cwd, entry.path));
};
...
const renameFile = (source, rename) => {
	const filename = path$5.basename(source, path$5.extname(source));
	const fileExtension = path$5.extname(source);
	const directory = path$5.dirname(source);
	if (typeof rename === 'string') {
		return path$5.join(directory, rename);
	}

	if (typeof rename === 'function') {
		return path$5.join(directory, `${rename(filename)}${fileExtension}`);
	}
...
function cpy(
	...
	*/
	const progressEmitter = new EventEmitter$1();
		...
		*/
		...
		for (const pattern of patterns) {
			...
			*/
			...
			entries = [
				...entries,
				...matches.map(sourcePath => new Entry(sourcePath, path$5.relative(options.cwd, sourcePath), pattern)),
			];
		}
		...
		*/
		...
		return pMap(
			...
			async entry => {
				...
				try {
					await cpFile$1.exports(entry.path, to, options).on(
						'progress',
						fileProgressHandler,
					);
				} catch (error) {
...
function transformFind(mode, infos) {
  return infos.filter((v) => v.specifier || v.expression).map((v) => {
    ...
    return {
      ...
      specifier,
      code: v.code,
      isNodeBuiltin: mlly.isNodeBuiltin(specifier)
    };
  });
}
function findExports(code) {
  return transformFind("export", mlly.findExports(code));
}
function findStaticImports(code) {
  return transformFind("import", mlly.findStaticImports(code));
}
function findDynamicImports(code) {
  return transformFind("dynamic", mlly.findDynamicImports(code));
}
function find(code) {
...
const mapAgeCleaner = /*@__PURE__*/getDefaultExportFromCjs(dist.exports);
...
const getRemoteStdVersion = mem(function() {
  return new Promise((resolve, reject) => {
    node_https.get("https://deno.land/std", (res) => {
...
async function defaultNormalize(payload) {
  let {
    code,
    npmCDN,
    content,
  ...
  if (isNodeBuiltin) {
    ...
    if (stdVersion === "autoGetRemote") {
      stdVersion = await getRemoteStdVersion();
    }
    if (!stdVersion) {
      return replace(
  ...
  const packageExists = localPkg.isPackageExists(specifier, {
    paths: [process.cwd()]
  });
  if (packageExists) {
    const packageInfo = await localPkg.getPackageInfo(specifier, {
      paths: [process.cwd()]
    });
...
function normalizeUrl(cdn) {
  return /\/$/.test(cdn) ? cdn : cdn + "/";
}
function createReplace(content, code, specifier) {
  return function replace(replaceValue) {
...
function createTransformTextFile(handle) {
  return async function transformTextFile(filepath) {
...
async function ensureFile(filepath) {
  ...
  await promises.mkdir(path$5.dirname(filepath), {
    recursive: true
  });
...
async function udeno(options = {}) {
  ...
  await parallel([
    cpy(src, depsDir, {
      flat: true,
      filter(file) {
        return file.relativePath !== path$5.normalize(index);
      }
    }),
    cpy(index, "./", {
      flat: true,
  ...
  const filepaths = await fastGlob(`${depsDir}/**/*.ts`);
...
async function generateVscodeSetting(vscode) {
  const { path } = vscode;
  ...
  await promises.writeFile(settingsFilePath, settingsText);
}
async function transformReadMe(payload) {
...
exports.createTransformTextFile = createTransformTextFile;
...
exports.generateVscodeSetting = generateVscodeSetting;
exports.getAugmentedNamespace = getAugmentedNamespace;
exports.getRemoteStdVersion = getRemoteStdVersion;
exports.log = log;
exports.normalizeUrl = normalizeUrl;
exports.readTextFile = readTextFile;
exports.reduce = reduce;
exports.transformReadMe = transformReadMe;
exports.udeno = udeno;
